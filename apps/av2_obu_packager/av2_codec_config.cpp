...
bool AV2CodecConfigurationBox::append_config_obu(const BaseOBU& obu) {
  if (!input_file_) {
    spdlog::error("append_config_obu: no input file bound");
    return false;
  }
  ...
  if (!*input_file_) {
    spdlog::error("Failed to seek to OBU at offset {}", static_cast<long long>(pos.start_pos));
    input_file_->clear();
...
std::vector<uint8_t> AV2CodecConfigurationBox::serialize() const {
  ...
  if (out.size() != 9) {
    spdlog::error("av2C prefix size = {} bytes (expected 9)", out.size());
    return {};
  }
  ...
  spdlog::debug("Serialized av2C: {} bytes (9 prefix + {} configOBUs)", out.size(),
                config_obus.size());
  return out;
}
