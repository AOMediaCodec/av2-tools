...
struct SequenceTransformConfig;
...
struct ColorConfig {
  ...
  uint32_t color_primaries = CP_UNSPECIFIED;
  uint32_t transfer_characteristics = TC_UNSPECIFIED;
  uint32_t matrix_coefficients = MC_UNSPECIFIED;
  uint32_t color_range = 0;
  uint32_t chroma_sample_position = CSP_UNSPECIFIED;
  uint32_t csp_present_flag = 0;
...
struct OperatingParametersInfo {
  ...
  bool parse(BitstreamReader& br, uint32_t buffer_delay_length_minus_1);
  json to_json() const;
};
...
struct SequenceInterConfig {
  ...
  uint32_t use_extra_ref_frames = 0;
...
struct AV2SequenceHeader {
  ...
  std::vector<uint32_t> operating_point_idc;
  std::vector<uint32_t> seq_level_idx;
  std::vector<uint32_t> seq_tier;
  std::vector<uint32_t> decoder_model_present_for_this_op;
  std::vector<OperatingParametersInfo> operating_parameters;
  std::vector<uint32_t> initial_display_delay_present_for_this_op;
  std::vector<uint32_t> initial_display_delay_minus_1;
  ...
  SequenceTransformConfig transform_config;
  ...
  uint32_t reduce_aspect_ratio = 0;
  uint32_t max_pb_aspect_ratio_log2_m1 = 0;
  uint32_t MaxAspectRatio = 8;

  uint32_t df_par_bits_minus2 = 0;

  uint32_t user_defined_qmatrix = 0;

  uint32_t scan_type_info_present_flag = 0;
  uint32_t scan_type_idc = 0;
  uint32_t fixed_cvs_pic_rate_flag = 0;
  uint64_t elemental_ct_duration_minus_1 = 0;
